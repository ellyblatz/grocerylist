const items = [
  ...
  function capitalizeNames() {
    ...
    const mappedArr = items.map((item) => {
        const singleLtr = item.name.slice(0,1)
            const upperLtr = singleLtr.toUpperCase()
            const upperItem = upperLtr + item.name.slice(1)
            return {id: item.id, name: upperItem, price: item.price, categoryId: item.categoryId, inventory: item.inventory}
    }) 
  ...
  function calculateTotalInventory() {
    ...
    const sum = items.reduce((acc, items)=> {
        acc = acc + items[items.id];
        return acc;
      }, 0); 
      console.log(sum);
  ...
  function logCartItems() {
   ...
   console.log(cart)
    ...
    cart.forEach((itemId)=> {
        const item = items.find((allItems) => {
            return itemId === allItems.id
        })
        if(item){
            console.log.apply(item.name)}
        })
    }
